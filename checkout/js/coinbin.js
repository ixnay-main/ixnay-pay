$(document).ready(function() {
	...
	gun.on('auth', function(){
		...
		document.getElementById("fillAddress").value = vendorWallet;
		document.getElementById("fillAmount").value = productAmount;
			...
			if(dvalue>=total){
				...
				tx2.broadcast(function(data){
					if($(data).find("result").text()=="1"){
						...
						gun.get("user").get(vendorWallet).get("orders").set({
							amounts: 1
						})
	...
	$("#walletSendBtn").click(function(){
		...
		if($("#walletSpend .has-error").length==0){
			...
			if(total<=balance){
				...
				var getFailedOrders = gun.get("user").get(vendorWallet).get("failedOrders").get("amounts")
